...
app = Flask(__name__)
# Double app decl in server.py trace, removing one
CORS(app)
    ...
    try:
        ...
        if slope is not None:
            ...
            with DATA_LOCK:
                ...
                if GLOBAL_GDF is not None and idx in GLOBAL_GDF.index:
                    ...
                    # Save incrementally
                    # We might skip saving to disk on Render since it's ephemeral,
                    # but let's try.
                    try:
                       GLOBAL_GDF.to_file(BASE_GEOJSON, driver='GeoJSON')
                    except:
                       pass # Ignore write errors on read-only FS
